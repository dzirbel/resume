...
function compile {
    ...
    pug --obj "{ 'css' : '$RESUME_CSS$CONTAINER_CSS', 'content' : '$RESUME', 'container' : true,  'title' : '$TITLE' }" < container.jade > ../resume.html
    pug --obj "{ 'css' : '$RESUME_CSS',               'content' : '$RESUME', 'container' : false, 'title' : '$TITLE' }" < container.jade > ../embed.html
    pug --obj "{ 'css' : '$RESUME_CSS$PDF_CSS',       'content' : '$RESUME', 'container' : false, 'title' : '$TITLE' }" < container.jade > ../pdf.html

    cat ../resume.html > ../index.html
    if hash wkhtmltopdf 2>/dev/null; then
        wkhtmltopdf -q --page-size Letter --dpi 96 --title "$TITLE" ../pdf.html ../resume.pdf
    fi
    rm ../pdf.html
