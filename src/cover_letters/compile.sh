...
function compile {
    ...
    do
        ...
        pug --obj "{ 'css' : '$RESUME_CSS$CONTAINER_CSS', 'content' : '$COVER_LETTER', 'container' : true,  'title' : '$TITLE' }" < ../container.jade > $filename.html
        pug --obj "{ 'css' : '$RESUME_CSS$PDF_CSS',       'content' : '$COVER_LETTER', 'container' : false, 'title' : '$TITLE' }" < ../container.jade > pdf.html
